...
object GraphLoader {
  ...
  private val _log: Logger = LoggerFactory.getLogger("GraphLoader")
  ...
  def loadGraphHashBased(graphPath: String, context: SparkContext) {
    ...
    if (hashedCount.compareTo(distHashedCount) != 0) {
      // Collisions existing
      _log.error("Collisions occured while hashing. The graph can not be loaded with hashing. Use the option -countBased to load the graph without hashing.")
    }
  }
