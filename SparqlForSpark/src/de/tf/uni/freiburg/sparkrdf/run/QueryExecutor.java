...
public class QueryExecutor {
	...
	ArgumentParser.parseInput(args); // 解析命令行参数

	Logger log = Logger.getLogger(QueryExecutor.class);
	Map<String, Long> operationDuration = new TreeMap<>();
	long resCount = 0;

	SparkFacade.createSparkContext(); // Facade面门(跟interface有点像，很多类组成的子系统，隔离了spark的东西)设计模式
	...
	//System.out.println("start*********************************************getGraph()");
	SparkFacade.abc();
	endLoading = System.currentTimeMillis() - startLoading;
	endLoading /= 1000;
	//System.out.println("The whole process takes time: " + endLoading + " s");
